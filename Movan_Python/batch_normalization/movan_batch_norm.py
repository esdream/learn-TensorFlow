...
train_op_norm, cost_norm, layers_inputs_norm = built_net(
...
for i in range(250):
    ...
    # train on batch
    sess.run([train_op, train_op_norm], feed_dict={
    ...
    if i % record_step == 0:
        ...
        cost_his_norm.append(
            sess.run(cost_norm, feed_dict={xs: x_data, ys: y_data}))

plt.ioff()
plt.figure()
plt.plot(np.arange(len(cost_his)) * record_step,
         np.array(cost_his), label='no BN')     # no norm
plt.plot(np.arange(len(cost_his)) * record_step,
         np.array(cost_his_norm), label='BN')   # norm
plt.legend()
plt.show()