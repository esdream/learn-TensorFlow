...
def preprocess_for_train(image, height, width, bbox):
    ...
    # area_range参数是截取时必须包含的图像范围
    # aspect_ratio_range参数是截取时高和宽的比例范围
    bbox_begin, bbox_size, _ = tf.image.sample_distorted_bounding_box(tf.shape(image), bounding_boxes=bbox, area_range=[0.05, 1], aspect_ratio_range=[0.85, 1.2])
