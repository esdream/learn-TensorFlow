...
def main(_):
    #训练数据集
    train_file_path = os.path.join(FLAGS.buckets, "train_face_data.tfrecords")
    #测试数据集
    test_file_path = os.path.join(FLAGS.buckets, "test_face_data.tfrecords")
    #模型存储名称
    ...
    x = tf.reshape(train_images, [-1, 1024])
    y = tf.matmul(x, W) + b
    ...
    x_test = tf.reshape(test_images, [-1, 1024])
